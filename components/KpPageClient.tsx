...
export default function KpPageClient() {
  const pdfRef = useRef<HTMLDivElement>(null);
  const [data, setData] = useState<KpData>(emptyKp());
  ...
  useEffect(() => {
    ...
    if (saved) setData((prev) => ({ ...prev, seller: saved }));
  }, []);
  ...
  function setClient(patch: Partial<BuyerInfo>) {
  ...
  async function handleDownload() {
    ...
    if (
      requireFields &&
      (!data.seller.name || !data.client.name || data.items.some((i) => !i.name))
    ) {
      setMessage(
        "Заполните отправителя, получателя и все позиции — или снимите галочку проверки."
      );
      return;
    }
    ...
    try {
      ...
      await downloadPdfFromElement(pdfRef.current, `kp-${data.number || "1"}-${data.date}.pdf`);
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8">
      <div className="mb-6">
        <h1 className="text-3xl font-bold text-slate-900">Коммерческое предложение</h1>
        <p className="mt-2 text-slate-600">КП в PDF за пару минут · бесплатно · без регистрации</p>
      </div>
      ...
      <div className="grid gap-8 lg:grid-cols-2">
        <div className="space-y-6">
          <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              <label className="block sm:col-span-2">
                <span className="mb-1 block text-xs font-medium text-slate-600">Вступление</span>
                <textarea
                  value={data.intro}
          ...
          <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
            <h2 className="mb-4 text-lg font-semibold">От кого (ваши реквизиты)</h2>
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              <Field label="Телефон" value={data.seller.phone} onChange={(v) => setSeller({ phone: v })} />
            </div>
          </section>

          <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
            <h2 className="mb-4 text-lg font-semibold">Кому</h2>
            <div className="grid gap-3 sm:grid-cols-2">
              <Field label="Название / ФИО" value={data.client.name} onChange={(v) => setClient({ name: v })} className="sm:col-span-2" />
              <Field label="ИНН" value={data.client.inn} onChange={(v) => setClient({ inn: v })} />
              <Field label="КПП" value={data.client.kpp} onChange={(v) => setClient({ kpp: v })} />
              <Field label="Адрес" value={data.client.address} onChange={(v) => setClient({ address: v })} className="sm:col-span-2" />
            </div>
          </section>

          <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
            <h2 className="mb-4 text-lg font-semibold">Позиции и условия</h2>
            ...
            <div className="mt-4 grid gap-3 sm:grid-cols-2">
              <Field label="НДС" value={data.vatNote} onChange={(v) => setData({ ...data, vatNote: v })} className="sm:col-span-2" />
              <Field
                label="Срок действия, дней"
                value={String(data.validDays)}
                onChange={(v) => setData({ ...data, validDays: Number(v) || 0 })}
                type="number"
              />
              <Field
              ...
              />
              <label className="block sm:col-span-2">
                <span className="mb-1 block text-xs font-medium text-slate-600">Заключение</span>
                <textarea
                  value={data.closing}
      ...
      >
        <KpPreview data={data} previewRef={pdfRef} />
      </div>
    </div>
  );
}
